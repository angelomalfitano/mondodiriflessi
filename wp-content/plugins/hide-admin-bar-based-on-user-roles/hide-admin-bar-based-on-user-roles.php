...
if ( ! function_exists( 'habbour_fs' ) ) {
	...
	function habbour_fs() {
		...
		if ( ! isset( $habbour_fs ) ) {
			...
			$habbour_fs = fs_dynamic_init( array(
				...
				'type'                => 'plugin',
				'public_key'          => 'pk_ce031ea3e7feca59d50135999f301',
				'is_premium'          => false,
				'has_addons'          => false,
				...
				'menu'                => array(
					'slug'           => 'hide-admin-bar-based-on-user-roles',
					'override_exact' => true,
					'first-path'     => 'options-general.php?page=hide-admin-bar-settings',
					'account'        => false,
					'parent'         => array(
	...
	function habbour_fs_settings_url() {
		return admin_url( 'options-general.php?page=hide-admin-bar-settings' );
	}

	habbour_fs()->add_filter('connect_url', 'habbour_fs_settings_url');
	habbour_fs()->add_filter('after_skip_url', 'habbour_fs_settings_url');
	habbour_fs()->add_filter('after_connect_url', 'habbour_fs_settings_url');
	habbour_fs()->add_filter('after_pending_connect_url', 'habbour_fs_settings_url');
 ...
 */
define( 'HIDE_ADMIN_BAR_BASED_ON_USER_ROLES', '3.5.0' );
