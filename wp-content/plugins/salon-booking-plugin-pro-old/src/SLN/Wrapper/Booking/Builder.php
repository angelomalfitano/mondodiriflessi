...
class SLN_Wrapper_Booking_Builder
{
    ...
    public function getDeposit(){
        ...
        if ($settings->isPaymentDepositFixedAmount()) {
                $deposit = min($this->getTotal(), $depositAmount);
        }
        else {
                $deposit = ($this->getTotal() / 100) * $depositAmount;
        }
        return $deposit;
    }

    public function getStatus(){
        return SLN_Enum_BookingStatus::PENDING_PAYMENT;
    }
    ...
    public function getToPayAmount($format = true, $include_fee = true)
    {
        $ret = $this->getDeposit() > 0 ? $this->getDeposit() : $this->getTotal();
    ...
    public function getTimeStringToChangeStatusFromPending() {
        ...
        if ( $plugin->getSettings()->get('pay_offset_enabled')) {
