        ...
        if (empty($this->cache)) {
           ...
           ", $from->format('Y-m-d'), $to->format('Y-m-d')));
            ...
            foreach($posts as $post) {
                if (!isset($this->cache[$post->meta_value])) {
                    $this->cache[$post->meta_value] = array();
                }
                $this->cache[$post->meta_value][] = $post;
            }
        }

        $posts = isset($this->cache[$date->format('Y-m-d')]) ? $this->cache[$date->format('Y-m-d')] : array();
