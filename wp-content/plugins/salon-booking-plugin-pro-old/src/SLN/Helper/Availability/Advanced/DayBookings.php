    ...
    protected function buildTimeslots()
    {
        ...
        foreach ($bookings as $booking) {
            ...
            foreach ($bookingServices->getItems() as $bookingService) {
                ...
                foreach ($times as $time) {
                    $time = $time->format('H:i');
                    if ($bookingService->getService() && $bookingService->getAttendant() && apply_filters('sln_build_timeslots_add_attendant_to_timeslot', true, $time, $bookingService, $booking, $this->allBookings)) {
