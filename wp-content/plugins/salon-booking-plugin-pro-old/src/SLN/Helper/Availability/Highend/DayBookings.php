        ...
        foreach ($bookings as $booking) {
            ...
            foreach ($bookingServices->getItems() as $bookingService) {
                ...
                foreach ($times as $time) {
                    $key = $time->format('H:i');
                    if ($time < $bookingService->getBreakStartsAt() || $time >= $bookingService->getBreakEndsAt()) {
                        if ($bookingService->getService() && $bookingService->getAttendant() && apply_filters('sln_build_timeslots_add_attendant_to_timeslot', true, $key, $bookingService, $booking, $this->allBookings)) {
