        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (!data.success) {
                    ...
                    $("#sln-update-user-message").html(alertBox).fadeIn(500);
					$('#prev_services').html(data.prev_service_html); 
...
function sln_manageAddNewService($) {
    ...
    $('button[data-collection="addnewserviceline"]').on("click", function(e) {
        ...
        $('#sln_booking_services').find('select').each(function(){
            if('0' !== $(this).val()){
                $(this).next().find('.select2-selection--single').removeClass('select2-selection--single-warning');
            }else{
                $(this).next().find('.select2-selection--single').addClass('select2-selection--single-warning');
            }
        });
        $(this)
            ...
            if (data.attendants) {
                ...
                if (elems)
                    elems
                        .text(function() {
                            return (
                                $(this).text() + " " + sln_customBookingUser.not_available_string
                            );
                        })
                        .parent()
...
function sln_changeServices($, selected){
    ...
    if(service_data == undefined){
        let attendant_select = $(selected).closest('.row').find('select[data-selection="attendant-selected"]');
    ...
    if(!attendant_select.length){
        sln_checkServices($);
        $(selected).closest('.row').find('.sln-alert--onremove').removeClass('hide');
    }
    attendant_select.find('option').remove();
    attendant_select.data('service', s_id);
    attendant_select.attr('name', '_sln_booking[attendants][' + s_id + ']');
    let html = '<option value="' + 0 + '">' + attendantsData[0] + "</option>";
...
function sln_bindServicesSelects(line){
    ...
    $(line).find('select[data-selection="service-selected"]').on("select2:open", function() {
        $(this).closest('.row').find('.sln-alert--onremove').addClass('hide');
...
function sln_bindAttendantSelects(line) {
    ...
    function bindChangeAttendantSelectsFunction() {
        ...
        sln_checkServices($);
        $(this).closest('.row').find('.sln-alert--onremove').removeClass('hide');
        if(save_is_disabled){
            $('#save-post').attr('disabled', true);
        }else{
            $('#save-post').removeAttr('disabled');
